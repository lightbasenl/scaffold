...
> = ({ children, className, variantColor, ...props }) => {
  const variant = variantColor ? styles[`is${ucfirst(variantColor)}`] : undefined;

  return (
    <button className={classNames(styles.button, variant, className)} {...props}>
      {children}
    </button>
  );
};
...
> = React.forwardRef(({ children, variantColor, className, ...props }, ref) => {
  ...
  return (
    <a ref={ref} className={classNames(styles.button, variant, className)} {...props}>
      {children}
    </a>
  );
});
